...
public class PaymentRessource {
	...
	public Response postPayment(JsonObject jsonObject) {
		...
		URI uri = builder.path(PaymentRessource.class, "getPayment").build(payment.getCheckNumber());
	...
	public Response putPayment(@PathParam(("id")) String id, JsonObject jsonObject) {
		...
		GenericEntity<Payment> entity = new GenericEntity<>(paymentService.findByCheckNumber(id).get(0), Payment.class);
	...
	public Response deletePayment(@PathParam("id") String id) {
		...
		if (payment == null) {
			...
			GenericEntity<Payment> entity = new GenericEntity<>(payment, Payment.class);
